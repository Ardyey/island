...
.defines(function(){
	...
	EntityBase = ig.Entity.extend({
		...
		bgm: new ig.Sound( 'media/sounds/bgm.ogg' ),
		...
		draw: function () {
			...
			this.font.draw(`HP: ${this.health}`, 10, 10);
			this.font.draw(`Wave: ${this.wave}`, cx2*3, 10);
			this.font.draw(`Enemies: ${this.enemyCount3}/${this.maxEnemyCount}`, 390, 50);
			this.font.draw(`High Score: W-${hiScore ? hiScore : 0 }`, 160, 10);
			if( this.health == 0 ) {
				this.bgm.stop();
				localStorage.setItem('score', this.wave);
				var startText = 'Game Over!';
				var gameOverText = 'Press Left Mouse Button \n to Proceed';
				this.gameEnd.draw( 0, 0 );
				this.font.draw( startText, cx, cy, ig.Font.ALIGN.CENTER);
				this.font.draw( gameOverText, cx, 400, ig.Font.ALIGN.CENTER);
			}
		},
